    ...
    <div id="provinceOverlay" class="province-overlay" aria-hidden="true">
        ...
        <section class="province-panel" role="dialog" aria-modal="true" aria-labelledby="provinceOverlayTitle">
            ...
            <div class="province-map-wrap">
                <svg class="indonesia-map" viewBox="0 0 1380 460" role="img" aria-labelledby="indonesiaMapTitle indonesiaMapDesc">
                    ...
                    @foreach($indonesiaProvinces as $province)
                        <a href="{{ url('/provinsi/'.$province['slug']) }}" class="province-map-link" data-province="{{ $province['name'] }}" data-count="0" data-url="{{ url('/provinsi/'.$province['slug']) }}">
                            <rect x="{{ $province['x'] }}" y="{{ $province['y'] }}" width="{{ $province['w'] }}" height="{{ $province['h'] }}" rx="12"></rect>
                            <text x="{{ $province['x'] + ($province['w'] / 2) }}" y="{{ $province['y'] + ($province['h'] / 2) + 5 }}" text-anchor="middle">{{ $province['code'] }}</text>
                        </a>
                    @endforeach
                </svg>
            ...
            <div class="province-grid" aria-label="Daftar provinsi Indonesia">
                @foreach($indonesiaProvinces as $province)
                    <a href="{{ url('/provinsi/'.$province['slug']) }}" class="province-item" data-province="{{ $province['name'] }}" data-count="0" data-url="{{ url('/provinsi/'.$province['slug']) }}">
